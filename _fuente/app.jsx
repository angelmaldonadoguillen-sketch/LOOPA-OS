    ...
    <div className={`app ${tweaks.sidebarCollapsed ? 'collapsed' : ''}`}>
      <aside className="sidebar">
        ...
        <nav className="nav" style={{ marginTop: 8 }}>
          {modules.map((m, i) => (
            <button key={m.id} className={`nav-item ${route === m.id ? 'active' : ''}`} onClick={() => m.id === 'nueva' && route === 'nueva' ? goNueva(null) : setRoute(m.id)} title={m.label}>
              ...
              {!tweaks.sidebarCollapsed && <span className="num">{pad2(i + 1)}</span>}
        ...
        <div className="sidebar-footer">
          {pwaPrompt && !tweaks.sidebarCollapsed && (
            <button className="btn primary" style={{ width: '100%', justifyContent: 'center', marginBottom: 12, borderRadius: 999 }} onClick={() => pwaPrompt.prompt().then(() => setPwaPrompt(null))}>⬇ Instalar app</button>
          )}
          {saveError
            ? <><span className="status-dot" style={{ background: 'var(--danger)', boxShadow: '0 0 6px var(--danger)' }} />{!tweaks.sidebarCollapsed && 'Error al guardar'}</>
            : window.USE_FB
              ? <><span className="status-dot" />{!tweaks.sidebarCollapsed && 'Online · sincronizado'}</>
              : <><span className="status-dot" style={{ background: 'var(--warn)', boxShadow: '0 0 6px var(--warn)' }} />{!tweaks.sidebarCollapsed && 'Modo local · este equipo'}</>}
        </div>
      </aside>
